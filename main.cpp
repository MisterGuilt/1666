...
void main_du_joueur(int * ,int *,int *, int *);
...
int random_Card(int index,int *cpt)
{
    int j;
    /*j=rand()%10+1;*/
   static int tab[50];
                ...
                while (continuerTour)
                    {
                        ...
                        //while (event.type == SDL_MOUSEMOTION)
                          //  {
                               SDL_WaitEvent(&event);
                        if (SDL_MOUSEBUTTONUP&&(ptr_position->x<750||ptr_position->x>820||ptr_position->y<410||ptr_position->y>530))
                        {
                            continuerTour =1;
                            //SDL_PollEvent(&event);
                            affichage_Plateau(ptr_screen);
                            affichageCartes(image_Cartes,ptr_position,  ptr_screen, carte_precedente, carte_actuel, tour);
                            //SDL_BlitSurface(image_Cartes[2], NULL, ptr_screen, ptr_position);
                            //ptr_position->x = event.button.x;
                            //ptr_position->y = event.button.y;
                        }
                            //SDL_BlitSurface(image_Cartes[2], NULL, ptr_screen, ptr_position);
                            ...
                            //}
                        else            ///le joueur a cliqué sur la carte à jouer
                        {
                            *tour=1;
                            oss<<"Tour : "<<(*tour);
                            //SDL_BlitSurface(image_Cartes[2], NULL, ptr_screen, ptr_position);
                            ...
                            main_du_joueur(carte_precedente, carte_actuel, carte_pioche, tour);
                            affichageCartes(image_Cartes,ptr_position,  ptr_screen, carte_precedente, carte_actuel, tour);
                            SDL_Flip(ptr_screen);
                            SDL_PollEvent(&event);
                            SDL_WaitEvent(&event2);
                            switch(event2.type)             // On teste le type d'événement
                            {
                                case SDL_QUIT:              // Si c'est un événement QUITTER
                                    //continuer = 0;     //On met le booléen à 0, donc la boucle va s'arrêter
                                    return 0;
                                ...
                                case SDL_MOUSEBUTTONUP :
                                    {
                                        ptr_position->x = event2.button.x;
                                        ptr_position->y = event2.button.y;
                                        //affichage_Plateau(ptr_screen);
                                        main_du_joueur(carte_precedente, carte_actuel, carte_pioche, tour);
                                        affichageCartes(image_Cartes,ptr_position,  ptr_screen, carte_precedente, carte_actuel, tour);
                                    }
                                /*case
                                    {
                                    affichage_Plateau(ptr_screen);
                                    affichageCartes(image_Cartes,ptr_position,  ptr_screen);
                                    SDL_BlitSurface(image_Cartes[5], NULL, ptr_screen, ptr_position);
                                    }*/
                                default :
                                break;
                                }
...
void affichageCartes(SDL_Surface **image_Cartes, SDL_Rect *pos, SDL_Surface *screen, int *carte_precedente,int *carte_actuel,int *tour)
{
    ...
    for (int i=0;i<5;i++)       ///joueur 1
    {
        if (i==0)
        {
            SDL_SetColorKey(image_Cartes[10], SDL_SRCCOLORKEY, SDL_MapRGB(image_Cartes[10]->format, 63, 72, 204));    /// transparence
        ...
        {
            SDL_SetColorKey(image_Cartes[carte_actuel[i+4]], SDL_SRCCOLORKEY, SDL_MapRGB(image_Cartes[i]->format, 63, 72, 204));    /// transparence
            SDL_BlitSurface(image_Cartes[carte_actuel[i+4]], NULL, screen, &position);
            position.x=position.x+100;
        }
    }
    position.x=300;
    ...
    for (int i=0;i<3;i++)            ///pioche, defausse
    {
        ...
        {   position_pioche.x=position.x+10;
            ...
            SDL_SetColorKey(image_Cartes[carte_actuel[10]], SDL_SRCCOLORKEY, SDL_MapRGB(image_Cartes[i]->format, 63, 72, 204));    /// transparence
    ...
    for (int i=0;i<5;i++)
    {
        if (i==0)       /// carte figure
        {
            SDL_SetColorKey(image_Cartes[11], SDL_SRCCOLORKEY, SDL_MapRGB(image_Cartes[11]->format, 63, 72, 204));    /// transparence
            SDL_BlitSurface(image_Cartes[11], NULL, screen, &position);
            position.x=position.x-200;
        }
        if (i==1)
        {
            SDL_SetColorKey(image_Cartes[carte_actuel[i]], SDL_SRCCOLORKEY, SDL_MapRGB(image_Cartes[i]->format, 63, 72, 204));    /// transparence
            SDL_BlitSurface(image_Cartes[carte_actuel[i]], NULL, screen, &position);      ///joueur 2
            position.x=position.x-100;
        }
        if (i==2)
        {
...
void main_du_joueur(int *carte_precedente,int *carte_actuel,int *carte_pioche, int *tour)       /// initialise les cartes des joueurs en fonction du tour
{
        if (*tour==0)
        {
            for (int i=0;i<=10;i++)
            {
               ...
               carte_precedente[i]=carte_actuel[i];     //on recupere la main des joueur pour permettre le retour en arriere
            }
        }
        if (*tour==1)
        {
            for (int i=0;i<=8;i++)
            {
                if (i==1)       //||i==3||i==2||i==1
                {
                    carte_actuel[10]=carte_actuel[i];   ///la 11eme carte du plateau correspond a la defausse
                    carte_actuel[i]=random_Card(i,tour);
                }
            }
        }
                ...
                if (event.button.y > 190 && event.button.y <= 280  && event.button.x > 100 && event.button.x <= 300)
                    {
                       ...
                       // SDL_Delay(200);
                        cont=EventProcessing(&position,screen, carte_precedente, carte_actuel,carte_pioche, &tour);
